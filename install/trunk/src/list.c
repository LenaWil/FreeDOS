...
dat_t *listbox(dat_t *dat_ary, int dat_count, char *fromdir)
{
    ...
    for(;;) {
        ...
        for(k = 19; k < 24; k++) for(l = 2; l < 80; l++) {
            gotoxy(l, k);
            putch(' ');
        }
        gotoxy(2, 20);
        if(i != dat_count) {
            ...
            if(isfile(lsmfile)) lsm_description(20, 2, 4, lsmfile);
            else cat_file(dat_ary[i].name, 4);
            chdir(thecwd);
        ...
        switch(ch.key) {
            ...
            case ' ':
                if(i != dat_count) {
                    dat_ary[i].rank = (toupper(dat_ary[i].rank) == 'Y') ?
                                      'N' :
                                      'Y';
                    break;
                }
                /* Else fall through to KEY_ENTER */
            /* Complete the selection, return the results */
            case KEY_ENTER:
                if(!select_yn(catgets(cat,
                                     SET_PROMPT_YN,
                                     MSG_CONTINUE,
                                     MSG_CONTINUE_STR),
                             yes, no, NULL, NULL)) {
        ...
        putch('[');
        if(wherex() != 38) gotoxy(wherex() + 1, wherey());
        else gotoxy(wherex() + 4, wherey());
        putch(']');

        if(i == dat_count) {
            gotoxy(37, centre + (dat_count / 5) + 2);
            cputs("<DONE>");
            gotoxy(38, centre + (dat_count / 5) + 2);
        } else {
            gotoxy(8 + (j * 13), centre + (i / 5));
