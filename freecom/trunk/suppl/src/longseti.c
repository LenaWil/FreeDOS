...
su(bsystem): longmath
ty(pe): A
sh(ort description): Assigns a signed 16bit integer to a 32bit number
lo(ng description): 
pr(erequistes): 
va(lue): none; \tok{num := (int)n}
re(lated to): longclr longset longsetu
...
void longseti(dword *num, unsigned n)
{	assert(num);
	num->hi = ((int)(num->lo = n) < 0)? -1: 0;
}
#endif
